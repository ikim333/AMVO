function [convective_term_u, convective_term_v] = convective(u, v, delta, N)

convective_term_u = zeros (N+2, N+2);
convective_term_v = zeros (N+2, N+2);
...
for i=2:(N+1)
    for j=2:(N+1)
        ...
        Fs=(v(i,j)*delta+v(i+1,j-1)*delta)/2;
        ...
        convective_term_v(i,j)=ve*Fe-vw*Fw+vn*Fn-vs*Fs;
